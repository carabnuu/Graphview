...
.range(["#F4A460", "#48D1CC	",
	...
	d3.selectAll(".nodes").attr("r",d=>{
		...
		if(this.nodes.indexOf(stripscript(d.name))>=0){
			temp_this_nodes[d.name]=[];
			temp_this_nodes[d.name].push(d.type);
		}
	})
			...
			if(i===j){
 			...
 			_this.locallayer.append('rect')
							...
							.on('mouseout', function(d) {
								...
								if (d.i==d.j) d3.select(this).style('fill', function(){ return nodeSizeScale(d.type); });
