...
Paths.prototype.Push = function (id) {//从matrix来
	...
	originData.forEach(function (d) {
		...
		//console.log(node["_groups"][0][0]["__data__"]["x"])
		var force_temp_id=this.force_temp_id;
		force_temp_id.push([node["_groups"][0][0]["__data__"]["x"],node["_groups"][0][0]["__data__"]["y"]])
		this.force_temp_id=force_temp_id;
		if (node.empty()) return;
		_this.data.push({
			r: node.attr('r'),
			x: matrix.x,
...
Paths.prototype.generate = function (d) {
	var _this = this;
	var result = [];
	console.log(this.force_temp_id)
